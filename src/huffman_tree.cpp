...
void reduce_nodes(std::vector<std::shared_ptr<Node>>& freq_vec, bool to_root = false)
{
    DEBUG_MSG("\nDebug: Branching nodes\nReceived nodes count:" << freq_vec.size() << "\n");
    std::vector<std::shared_ptr<Node>>::iterator it = freq_vec.begin();
    std::vector<std::shared_ptr<Node>> ret_vec;
    while (it != freq_vec.end())
    {
        std::shared_ptr<Node> node1 = std::make_shared<Node>();
        std::shared_ptr<Node> node2 = std::make_shared<Node>();
        std::shared_ptr<Node> sum_node = std::make_shared<Node>();
        node1 = *it;
        ++it;
        if (it != freq_vec.end())
        {
            ...
            ret_vec.push_back(sum_node);
        }
        else
        {
            ret_vec.push_back(node1);
        }
        if (it != freq_vec.end())
        {
            ++it;
...
std::shared_ptr<Node> HuffmanTree::build_tree(std::vector<std::pair<char, int>> freq_vec)
{
    DEBUG_MSG("\nDebug: Building tree\n");
    if (freq_vec.empty())
    {
...
const std::string HuffmanTree::extract_from_tree(const std::string binary_bits, std::shared_ptr<Node>& node)
{
...
void HuffmanTree::navigate_from_node(std::shared_ptr<Node>& node, int indent = 1)
{
...
void HuffmanTree::build_map(std::shared_ptr<Node>& node, std::string bits, std::map<const char, const std::string>& op_map)
{
    if (node->ch != '\0')
    {
